...
# --- Testes para extract_exchange_data ---
@patch('extract.extract_api_exchange.requests.get')
def test_extract_exchange_data_success(mock_get):
    mock_response = MagicMock()
    mock_response.json.return_value = [{"cotacaoCompra": 5.0, "cotacaoVenda": 5.1}]
    mock_get.return_value = mock_response

    url = "https://api-falsa.com/dolar"
    result = extract_exchange_data(url)
...
@patch('extract.extract_api_exchange.requests.get')
def test_extract_exchange_data_timeout(mock_get, caplog):
    mock_get.side_effect = requests.exceptions.Timeout("Timeout forced")

    with caplog.at_level(logging.ERROR):
        result = extract_exchange_data("https://api-falsa.com/dolar")
...
@patch('extract.extract_api_exchange.requests.get')
def test_extract_exchange_data_connection_error(mock_get, caplog):
...
@patch('extract.extract_api_exchange.requests.get')
def test_extract_exchange_data_http_error(mock_get, caplog):
    mock_response = MagicMock()
...
@patch('extract.extract_api_exchange.requests.get')
def test_extract_exchange_data_empty_return(mock_get, caplog):
    mock_response = MagicMock()
...
# --- Testes para save_raw_exchange_data ---
@patch('extract.extract_api_exchange.Path.mkdir')
@patch('builtins.open', new_callable=mock_open)
@patch('extract.extract_api_exchange.json.dump')
def test_save_raw_exchange_data(mock_json_dump, mock_file_open, mock_mkdir, caplog):
    dummy_data = [{"cotacaoCompra": 5.0, "cotacaoVenda": 5.1}]

    with caplog.at_level(logging.INFO):
        save_raw_exchange_data(dummy_data)
