...
@patch("extract.extract_db_tables.create_db_connection")
def test_extract_table_data_success(mock_create_db_connection, mock_read_sql):
    mock_engine = MagicMock()
    mock_create_db_connection.return_value = mock_engine
    ...
    mock_read_sql.assert_called_once_with("usuarios", mock_engine)


@patch("extract.extract_db_tables.pd.read_sql")
@patch("extract.extract_db_tables.create_db_connection")
def test_extract_table_data_empty(mock_create_db_connection, mock_read_sql, caplog):
    mock_create_db_connection.return_value = MagicMock()
    ...
    assert result.empty
    assert "Nenhum dado foi retornado do banco de dados" in caplog.text


@patch("extract.extract_db_tables.pd.read_sql")
@patch("extract.extract_db_tables.create_db_connection")
def test_extract_table_data_operational_error(mock_create_db_connection, mock_read_sql, caplog):
    mock_create_db_connection.return_value = MagicMock()

    mock_read_sql.side_effect = sqlalchemy.exc.OperationalError(
        "query", "params", Exception("orig")
    )
...
@patch("extract.extract_db_tables.pd.read_sql")
@patch("extract.extract_db_tables.create_db_connection")
def test_extract_table_data_interface_error(mock_create_db_connection, mock_read_sql, caplog):
    mock_create_db_connection.return_value = MagicMock()

    mock_read_sql.side_effect = sqlalchemy.exc.InterfaceError(
        "query", "params", Exception("orig")
    )
...
@patch("extract.extract_db_tables.pd.read_sql")
@patch("extract.extract_db_tables.create_db_connection")
def test_extract_table_data_programming_error(mock_create_db_connection, mock_read_sql, caplog):
    mock_create_db_connection.return_value = MagicMock()

    mock_read_sql.side_effect = sqlalchemy.exc.ProgrammingError(
        "query", "params", Exception("orig")
    )
